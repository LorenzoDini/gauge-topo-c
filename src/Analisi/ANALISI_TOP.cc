...
Data::Data(void)
  ...
  medie  = new double [4*numobs];
  errori = new double [4*numobs];

  jacksample = new double * [numblocks];
  for(i=0; i<numblocks; i++)
     {
     jacksample[i]=new double [4*numobs];
     }
  }
...
void Data::initfromfile(std::string nome_file)
  ...
  scalefactor=new double [(numobs-3)/2];

  filein.open("scale_factor.dat", std::ios::in);
  if(filein.is_open())
    {
    ...
    {
    std::cerr << "Error in opening the file scale_factors.dat\n";
    exit(1);
    }
  filein.close();
  ...
  if(filein.is_open())
    ...
    for(i=0; i<sample; i++)
       ...
       for(j=0; j<(numobs-3)/2; j++)
         {
         filein >> temp1;
         dati[i][3+2*j]=myround(temp1*scalefactor[j]); // "corrected" cooled top. charge

         filein >> temp1;
         dati[i][3+2*j+1]=temp1; // cooled average plaquette
         }
       }
    filein.close();
    }
...
void Data::jacksamplegen(void)
  ...
  int j;
  double* h; // for helper
  const double invvol=1.0/(double) ( d_latox*d_latoy*d_latoz*d_latot );

  h = new double[4*numobs];

  // initialize to zero medie
  for(j=0; j<4*numobs; j++)
     {
     medie[j]=0.0;
     }
  // sum up all data
  for(i=0; i<numblocks*block; i++)
     ...
     for(j=0; j<numobs; j++)
        ...
        medie[4*j+3]+=dati[i][j]*dati[i][j]*dati[i][j]*dati[i][j]*dati[i][j]*dati[i][j]; // Q^6
       }
     }
  // generate jacknife samples
  for(b=0; b<numblocks; b++)
     ...
     for(i=b*block; i<(b+1)*block; i++)
        ...
        for(j=0; j<numobs; j++)
           ...
           h[4*j+3]-=dati[i][j]*dati[i][j]*dati[i][j]*dati[i][j]*dati[i][j]*dati[i][j]; // Q^6
           } 
        }
     // normalize helper
     for(j=0; j<4*numobs; j++)
     ...
     for(j=0; j<numobs; j++)
        ...
        jacksample[b][4*j+1] = h[4*j+1]*invvol; // <Q^2>/vol
...
void Data::printobs(std::string nome_file)
  ...
  file1.open (nome_file.c_str(), std::ios::out);
  file1 << "  " << d_latox;
  file1 << "  " << d_latoy;
  file1 << "  " << d_latoz;
  file1 << "  " << d_latot;
